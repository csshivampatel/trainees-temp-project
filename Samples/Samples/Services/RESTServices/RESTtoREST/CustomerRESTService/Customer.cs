...
namespace Neuron.Samples.CustomerREST
{
    public class CustomerService : ICustomerService
    {
        ...
        bool ICustomerService.AddCustomer(Customer customer)
        {
            ...
            {
                var exResult = new ErrorResult(false, "An unknown error occurred");
                throw new WebFaultException<ErrorResult>(exResult, System.Net.HttpStatusCode.InternalServerError);
            }
            ...
            var result = new ErrorResult(false, String.Format("Customer with Id {0} already exists", customer.ID));
            throw new WebFaultException<ErrorResult>(result, System.Net.HttpStatusCode.Conflict);
        }
