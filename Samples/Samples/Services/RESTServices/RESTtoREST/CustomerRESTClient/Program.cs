        ...
        public static void CallRestService()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    JObject jsonResponse = JObject.Parse(addCustomerResponse.Content.ReadAsStringAsync().Result);
                    Console.WriteLine(String.Format("Response status code = {0}, Reason Phrase = {1}, Response = {2}", addCustomerResponse.StatusCode, addCustomerResponse.ReasonPhrase, jsonResponse));
                }
                ...
                {
                    JArray jsonResponse = JArray.Parse(getAllCustomersResponse.Content.ReadAsStringAsync().Result);
                    foreach (var cust in jsonResponse)
                        Console.WriteLine("Customer ID = {0}, Name = {1}, Email = {2}", cust["id"], cust["name"], cust["email"]);
                }
                else
                {
                    JObject jsonResponse = JObject.Parse(getAllCustomersResponse.Content.ReadAsStringAsync().Result);
                    Console.WriteLine("There was as error retrieving all customers");
                    Console.WriteLine(String.Format("Response status code = {0}, Reason Phrase = {1}, Response = {2}", getAllCustomersResponse.StatusCode, getAllCustomersResponse.ReasonPhrase, jsonResponse));
                }
